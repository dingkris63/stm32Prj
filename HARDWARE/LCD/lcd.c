...
u16 PEN_COLOR = 0x0000; // 画笔颜色
u16 BG_COLOR = 0xFFFF;  // 背景色
...
void LCD_WR_REG(vu16 regval) {
  regval = regval;       // insert this expression for delay if add -O2
  LCD->LCD_REG = regval; // Register ID
}

// Write LCD data
void LCD_WR_DATA(vu16 data) {
  data = data; // add this expression for delay if add -O2
  LCD->LCD_RAM = data;
}

// Read LCD Data
u16 LCD_RD_DATA(void) {
  vu16 ram; // __IO uint16_t
  ram = LCD->LCD_RAM;
  return ram;
}
...
void LCD_WriteReg(u16 LCD_Reg, u16 LCD_RegValue) {
  LCD->LCD_REG = LCD_Reg;      // Register ID
  LCD->LCD_RAM = LCD_RegValue; // Data
}
// Read Register
// LCD_Reg: Register Address
// LCD_RegValue: Data
u16 LCD_ReadReg(u16 LCD_Reg) {
  LCD_WR_REG(LCD_Reg); // Register ID
  delay_us(5);
  return LCD_RD_DATA();
}
// 开始写GRAM
...
void LCD_Init(void)
{
  ...
  lcddev.id|=LCD_RD_DATA();
	...
	if(lcddev.id==0X5310)//如果是这几个IC,则设置WR时序为最快
	{
		//重新配置写时序控制寄存器的时序
...
static void delay_ms(uint32_t ms) {
  ...
  TIM2->ARR = ms;
  TIM2->CNT = 0;
  TIM2->SR &= ~TIM_SR_UIF;  // Clear flag (in case it was set)
...
static void delay_us(uint32_t us) {
  TIM2->PSC = SystemCoreClock / 1000000 - 1; // 1?s per tick
  TIM2->ARR = us;                            // Set delay duration
