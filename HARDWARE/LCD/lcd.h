...
typedef struct {
  ...
  u16 id;      // LCD ID
  u8 dir;      // direction：0，vertical；1: harimental。
  u16 wramcmd; // graphics ram command
  u16 setxcmd; // setting x-coordinate command
  u16 setycmd; // setting y-coordinate command
} _lcd_dev;

extern _lcd_dev lcddev;

extern u16 PEN_COLOR; // default is red
extern u16 BG_COLOR;  // background color, defualt is white
...
#define PBout(x) BIT_ADDR((GPIOB_BASE+20), x)
#define LCD_LED PBout(15) // Back light led.
// LCD地址结构体
typedef struct {
  vu16 LCD_REG;
  vu16 LCD_RAM;
